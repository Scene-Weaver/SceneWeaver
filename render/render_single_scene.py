...
def add_light(size = 100,strength=10):
    ...
    light.energy = light_area * strength  # 500 watts per square meter (adjust as needed)

    # Position the light
    light_obj.location = ((min_x+max_x)/2, (min_y+max_y)/2, 4)   # Above the scene (X,Y,Z)

    # Make it visible in renders
#    light.cycles.use_multiple_importance_sampling = True

    print(f"Created {light.size}x{light.size_y}m area light with {light.energy:.0f}W power")
    
def create_plane(name="MyPlane", 
                location=(0, 0, 0), 
                rotation=(0, 0, 0), 
                scale=(1, 1, 1)):
    """
    ...
    plane.rotation_euler = (radians(rotation[0]), 
                           radians(rotation[1]), 
                           radians(rotation[2]))
    plane.scale = scale
...
if __name__ == "__main__":
    ...
    delete_collections_and_objects_by_name_keywords(["wall", "window", "placeholder", "bbox"])
#    add_light(strength=20)
    add_wall()
    center_scene()
    setup_camera(resolution=1024)
    anchor = setup_rotation_anchor()
    bpy.context.scene.render.engine = 'BLENDER_EEVEE_NEXT' #'CYCLES' #'BLENDER_EEVEE_NEXT'
    #print("Current render engine:", bpy.context.scene.render.engine)
    render_views(anchor, angles_deg=range(0,360,45),output_dir=roomdir,render_type="eevee")
